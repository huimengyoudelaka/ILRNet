...
def geometric_approximation_1(s):  
    ...
    I = torch.ones(shape, device=s.device).type(dtype)
    temp = 1e-8 * I
    s = s + temp
    ...
    a1 = s.unsqueeze(-1).repeat(1, 1, 1, shape[-1])
    # a1 = s.unsqueeze(-1).repeat(1, 1, 1, shape[-1]).permute(0,1,3,2)
    # a1 = s.repeat(s.shape[0], 1).t()
    a1_t = a1.permute(0,1,3,2)
...
class svdv2_1(torch.autograd.Function):
    ...
    def backward(ctx, dL_du, dL_ds,dL_dv):
        ...
        sym_M = K_t*torch.matmul(VT,dL_dv)
        sym_MT = sym_M.permute(0,1,3,2)
        sym = 0.5*(sym_M+sym_MT)
        tt=2*torch.matmul(diag_s,sym)
