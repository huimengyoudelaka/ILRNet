...
class SRUREDC3D_attention_dsv(nn.Module):
    def __init__(self, in_channels, channels, num_half_layer, sample_idx, has_ad=True, bn=True, plain=False,if_fe=True):
        ...
        self.reconstructor = DoSRUconv(channels*num_half_layer, in_channels,k=3, s=1, p=1, bias=True, bn=bn, model='deconv')
        self.if_fe = if_fe
...
class SRU3DDecoder(nn.Module):
    ...
    def forward(self, x, xs,outshape, reverse=False):  
        ...
        if not self.enable_ad:
            ...
            dsv.append(self.dsv_layers[0](x,outshape))
            ...
            g = xs.pop()
            x = torch.cat([g,x],dim=1)
            dsv.append(self.dsv_layers[-1](x,outshape))
            return torch.cat(dsv,dim=1)