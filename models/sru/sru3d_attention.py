...
from .grid_attention_layer import MultiAttentionBlock,UnetGridGatingSignal3,GridAttentionBlock3D
act = 'tanh'
AttentionBlock=GridAttentionBlock3D
...
class SRUREDC3D_attention(nn.Module):
    def __init__(self, in_channels, channels, num_half_layer, sample_idx, has_ad=True, bn=True, plain=False,if_fe=True,attention_dsample=(2,2,2)):
        print(if_fe)
        super(SRUREDC3D_attention, self).__init__()
        ...
        self.decoder = SRU3DDecoder(channels*(2**len(sample_idx)), num_half_layer, sample_idx, has_ad=has_ad, bn=bn, plain=plain,attention_dsample=attention_dsample)
        
        self.reconstructor = DoSRUconv(channels*2, in_channels,k=3, s=1, p=1, bias=True, bn=bn, model='deconv')
        self.if_fe = if_fe
...
class SRU3DDecoder(nn.Module):
    def __init__(self, channels, num_half_layer, sample_idx, is_2d=False, has_ad=True, bn=True, plain=False,attention_dsample=(2,2,2)):
        ...
        for i in reversed(range(num_half_layer)):
            if i == num_half_layer-1:
                ...
                attentionblock = AttentionBlock(channels, channels, channels,
                                                   nonlocal_mode, attention_dsample)
                #attentionblock = AttentionBlock(in_size=channels, gate_size=channels, inter_size=channels,
                ...
                gate_channels = channels
                    ...
                    attentionblock = AttentionBlock(channels, gate_channels, channels,
                                                   nonlocal_mode, attention_dsample)
            else:
                    if not plain:
                        decoder_layer = SiSRUconv(channels*2, channels//2,k=3, s=1, p=1, upsample=(1,2,2), bn=bn,model = 'upsampleconv')
                        attentionblock = AttentionBlock(channels//2, gate_channels, channels//2,
                                                   nonlocal_mode, attention_dsample)
                    else:
